# SynemaGuard Research Reference
## Comprehensive Survey: AI Governance Layers, Safety Guardrails & Deontological Agent Constraints

> **Purpose:** This document maps every relevant tool, open-source framework, arXiv paper, benchmark, and enterprise product that is directly applicable to building SynemaGuard — a deterministic, rule-based ethical orchestration layer sitting atop generative AI pipelines (LLMs + Diffusion models) for the "Cartoon Movie" project.

---

## TABLE OF CONTENTS

1. [The Conceptual Foundation — Why SynemaGuard is Novel](#1-conceptual-foundation)
...
9. [SynemaGuard Architecture Mapping](#9-architecture-mapping)

---

## 1. CONCEPTUAL FOUNDATION

SynemaGuard's core tension is described precisely in the paper *"Toward a Safe Internet of Agents"* (arXiv:2512.00520):
...
| **AI Control (SynemaGuard's approach)** | Force the system to be safe via external constraints | Must be architecturally sound; cannot rely on the model |

SynemaGuard operates in the **AI Control** paradigm — analogous to how a type system in a compiler enforces correctness regardless of programmer intent.

The **policy-to-practice gap** (arXiv:2509.23994) is the central problem: high-level human-readable rules must become low-level machine-enforceable constraints at every node in the pipeline. This is exactly what SynemaGuard's Circuit Breakers address.
...
- **Relevance to SynemaGuard:** Direct analog for the Script Agent layer. Colang scripts can encode your character bible rules and narrative constraints. Integrates natively with **LangChain/LangGraph**.
- **Key features:**
...
- **Relevance to SynemaGuard:** Perfect for the Script Agent's structural rules. Can validate that a script output contains required fields (scene number, character names from the bible, no prohibited phrases), enforcing schema compliance rather than relying on the model to comply.
- **Key features:**
...
- **Relevance to SynemaGuard:** The most architecturally relevant open-source tool. AlignmentCheck monitors whether agent actions remain aligned with intended goals across multi-step workflows — directly applicable to your circuit breaker concept.
- **Key features:**
...
- **Relevance to SynemaGuard:** Can be composed as individual scanner modules at each pipeline node — one scanner per rule category in your character bible or visual safety spec.
- **Key features:**
...
- **Relevance to SynemaGuard:** The "trace-level" enforcement model is exactly what circuit breakers need — the ability to halt a workflow when cumulative behavior crosses a threshold.
...
> **SynemaGuard Recommendation:** Run **LlamaGuard 4** or **NeMoGuard-8B** as the classifier at the output of both the Script Agent and Visual Generation Agents. These are fast, local, and can be fine-tuned on your character bible vocabulary.
...
- **Relevance:** Critical for SynemaGuard's audit log requirement. Every node decision — pass/block/halt — is logged and traceable.
...
- **Direct Relevance:** SynemaGuard's three-layer approach (Script → Visual → Circuit Breaker) maps directly to pre/in/post-planning stages. The Agent Constitution concept is the theoretical basis for your character bible enforcement.
...
- **Direct Relevance:** Blueprint for SynemaGuard's inter-agent communication security. The PromptGuard + AlignmentCheck combination is equivalent to your input gate + circuit breaker.
...
- **Direct Relevance:** Identical to SynemaGuard's deontological premise — agents can only do what their rule set explicitly permits, not what they statistically infer is helpful.
...
- **Direct Relevance:** Exactly the engineering challenge SynemaGuard must solve: your character bible and visual safety specs are human-readable policies; SynemaGuard must translate them to runtime constraints.
...
- **Direct Relevance:** Full architectural reference for SynemaGuard's design. The "guardrail architecture" section (3.5) is essential reading.
...
- **Direct Relevance:** This paper is the theoretical manifesto for SynemaGuard's deontological approach. Directly validates why SynemaGuard sits *outside* the agents (not inside them).
...
- **Direct Relevance:** Validates SynemaGuard's cascade risk framing. The Script Agent → Visual Agents cascade you identified is a known failure mode with formal characterization here.
...
- **Direct Relevance:** Provides metrics for measuring SynemaGuard's effectiveness at the Script Agent layer.
...
- **Direct Relevance:** This is the blueprint for SynemaGuard's Visual Agent IP protection layer. The three-component architecture can be directly integrated as a pre-diffusion gate.
...
- **Direct Relevance:** CopyJudge's LVLM-judge architecture can serve as SynemaGuard's output validator for the visual pipeline.
...
- **Direct Relevance:** Understanding the attack vector helps design defenses. SynemaGuard needs to assume the underlying diffusion model *may already* memorize protected content.
...
- **Direct Relevance:** Provides a quantitative audit methodology — SynemaGuard's logging layer can track infringement risk scores per generation.
...
- **Direct Relevance:** Establishes the baseline risk that SynemaGuard must defend against.
...
- **Direct Relevance:** Use this benchmark to test SynemaGuard's Visual Agent safety layer before deployment.
...
- **Direct Relevance:** Use as a threat model reference for enumerating all possible failure modes SynemaGuard must address.
...
- **Direct Relevance:** LangGraph's directed graph topology defines SynemaGuard's threat surface. G-Safeguard's methods apply directly.
...
- **Direct Relevance:** SynemaGuard's moderation layer should be adaptive — new character bible rules or new IP protections should be addable without retraining.
...
- **Direct Relevance:** The "filter agent" concept is SynemaGuard's inter-node sanitization layer.

---

## 7. SAFETY BENCHMARKS & EVALUATION DATASETS

Use these to test, red-team, and validate SynemaGuard's performance:
...
SynemaGuard's deontological layer must be compatible with these external requirements:

### Technical Standards

| Standard | Body | Relevance to SynemaGuard |
|---|---|---|
| **NIST AI Risk Management Framework (AI RMF)** + **NIST.AI.600-1** (GenAI Profile, 2024) | NIST | Mandates role-based access, continuous monitoring, adversarial testing, and lifecycle logging — all implemented by SynemaGuard's circuit breakers and audit logs |
...
| **OWASP Top 10 for LLM Applications v2025** | OWASP | Catalogs exact agent failure classes: prompt injection, tool misuse, memory leakage. SynemaGuard must address all 10. |

### Regulatory Frameworks

| Regulation | Jurisdiction | Key Requirements for SynemaGuard |
|---|---|---|
...
- **EU Copyright Directive (Art. 4 TDM exception)** — Opt-out mechanisms for training data. SynemaGuard's IP filter must respect registered opt-outs.
- **Getty Images v. Stability AI (2023, ongoing)** — Key case establishing that commercial diffusion models can be held liable for training on protected images. SynemaGuard's visual safety layer is a direct risk mitigation.

---

## 9. SYNEMA GUARD ARCHITECTURE MAPPING
...
| SynemaGuard Component | Recommended Tool |
|---|---|
